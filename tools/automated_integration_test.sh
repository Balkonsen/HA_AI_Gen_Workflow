...
log_function_entry() {
    ...
    if [ "${TRACE_FUNCTIONS:-false}" = true ]; then
        echo -e "${MAGENTA}â†’ ${func_name}${NC}" >&2
    fi
}
...
section() {
    local title="$*"
    log_both ""
    log_both "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
    log_both "  ${title}"
    log_both "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
    log_both ""
}
...
test_api_error_messaging() {
    ...
    if grep -q 'â”â”â”â”â”â”' "${REPO_ROOT}/ha_ai_workflow_addon/run.sh"; then
...
EOF
    ...
    if [ "${AUTO_TOKEN}" = true ] && [ -n "${SUPERVISOR_TOKEN:-}" ]; then
        ...
        echo ""
        echo "To create one: Home Assistant â†’ Profile â†’ Security â†’ Long-Lived Access Tokens"
        echo ""
    ...
    if [ ${FAILED_TESTS} -eq 0 ]; then
        log_both "${GREEN}âœ“ ALL TESTS PASSED${NC}"
        log_both ""
        ...
        log_both "${RED}âœ— SOME TESTS FAILED${NC}"
        log_both ""
...
EOF
    ...
    if [ ${FAILED_TESTS} -eq 0 ]; then
        echo "âœ… **ALL TESTS PASSED** - Installation validated successfully" >> "${REPORT_FILE}"
    else
        echo "âŒ **SOME TESTS FAILED** - Please review failures above" >> "${REPORT_FILE}"
    fi
...
1. âœ… File naming consistency (setup.sh)
2. âœ… SUPERVISOR_TOKEN validation (run.sh)
3. âœ… BIN_DIR fallback error handling (ha_ai_master_script.sh)
4. âœ… Expanded .env search paths (ha_ai_master_script.sh)
5. âœ… Command execution testing (setup.sh)
6. âœ… Improved API error messaging (run.sh)
7. âœ… Python 3.13 in CI matrix (ci-cd.yml)
