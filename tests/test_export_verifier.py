...
class TestVerifyDevices:
    ...
    def test_verify_devices_success(self, temp_dir):
        ...
        device_data = {
            "total_devices": 5,
            "devices_by_manufacturer": {"Philips": 2, "Sonos": 1},
            "devices_by_integration": {"hue": 2, "sonos": 1},
        }
...
class TestRun:
    ...
    def test_run_complete_export(self, temp_dir):
        ...
        diag_dir = Path(temp_dir) / "diagnostics"
        (diag_dir / "entities_registry.json").write_text(
            json.dumps(
                {
                    "total_entities": 5,
                    "entities_by_domain": {},
        ...
        (diag_dir / "devices_registry.json").write_text(
            json.dumps(
                {
                    "total_devices": 3,
        ...
        secrets_dir = Path(temp_dir) / "secrets"
        (secrets_dir / "secrets_map.json").write_text(json.dumps({"total_secrets": 2, "secrets": {}}))
...
class TestGenerateReport:
    ...
    def test_generate_report(self, temp_dir):
        ...
        verifier = ExportVerifier(temp_dir)
        verifier.stats = {"entities": {"total": 10}, "devices": {"total": 5}}
