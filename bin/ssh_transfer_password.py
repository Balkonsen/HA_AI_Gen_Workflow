...
class SSHAuthValidator:
    ...
    @staticmethod
    def validate_auth_config(host: str, user: str, port: int, 
                           key_path: Optional[str] = None,
                           password: Optional[str] = None) -> Tuple[str, List[str]]:
...
class SSHTransferPasswordSupport:
    ...
    def __init__(self, host: str, user: str = "root", port: int = 22,
                 ...
                 auth_method: str = "auto", docker_config: Optional[dict] = None):
        ...
        if auth_method == "auto":
            self.auth_method, _ = SSHAuthValidator.validate_auth_config(
                host, user, port, key_path, password
            )
    ...
    def _execute_with_interactive(self, cmd: List[str]) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                text=True,
                timeout=300  # Longer timeout for interactive
            )
            return result
        except subprocess.TimeoutExpired:
    ...
    def test_connection(self) -> Tuple[bool, str]:
        ...
        try:
            if self.auth_method == "paramiko":
                ...
                return result.returncode == 0, "Interactive connection successful" if result.returncode == 0 else "Interactive connection failed"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', choices=['auto', 'key', 'password', 'paramiko', 'sshpass', 'interactive'],
